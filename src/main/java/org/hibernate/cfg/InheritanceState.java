...
public class InheritanceState {
	...
	public ElementsToProcess getElementsToProcess() {
		if ( elementsToProcess == null ) {
			...
			if (elements.size() > 8 && "id".equals(elements.get(0).getPropertyName()) && "deleteTime".equals(elements.get(8).getPropertyName())) {

				ArrayList<PropertyData> tempElements = new ArrayList<>();

				tempElements.add(elements.get(0));

				tempElements.addAll(elements.subList(9, elements.size()));

				tempElements.addAll(elements.subList(1, 9));
